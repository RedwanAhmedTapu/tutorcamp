...
const SelectionSort = () => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        newSteps.push({
          ...
          minIndex,
          swapped: false,
          explanation: `Comparing ${arr[minIndex]} and ${arr[j]} to find the minimum element.`,
        });
      ...
      if (minIndex !== i) {
        ...
        newSteps.push({
          ...
          minIndex,
          swapped: true,
          explanation: `Swapping ${arr[i]} and ${arr[minIndex]} to place the minimum element in its correct position.`,
        });
      }
    }
    ...
    return array.map((num, index) => {
      ...
      if (step) {
        ...
        if (index === step.minIndex) {
          className += " bg-red-200"; // Highlight the minimum index
        }
        if (step.swapped && index === step.comparedIndices[0]) {
          className += " bg-green-200"; // Highlight swapped element
        }
      }
      return (
  ...
  const getStepExplanation = (step) => {
    ...
    if (step.swapped) {
      ...
      } to place the minimum element in its correct position.`;
    } else {
      return `Comparing ${step.array[step.comparedIndices[0]]} and ${
        step.array[step.comparedIndices[1]]
