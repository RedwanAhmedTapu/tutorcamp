  ...
  const heapify = (arr, n, i, steps) => {
    ...
    if (largest !== i) {
      ...
      steps.push({
        ...
        comparedIndices: [i, left, right],
        swapped: false,
        explanation: `Heap property maintained at index ${i}.`,
      });
    }
  };
  ...
  const heapSort = () => {
    ...
    for (let i = n - 1; i > 0; i--) {
      ...
      newSteps.push({
        ...
        comparedIndices: [0, i],
        swapped: true,
        explanation: `Swapped root (${arr[0]}) with last element (${arr[i]}) to place the largest element at the end.`,
      });
  ...
  const renderArray = () => {
    ...
    return array.map((num, index) => {
      ...
      if (step) {
        ...
        if (index === step.heapIndex) {
          className += " bg-red-200"; // Highlight heap index
        }
