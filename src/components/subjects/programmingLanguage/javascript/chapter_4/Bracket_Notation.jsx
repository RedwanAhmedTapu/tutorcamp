...
const BracketNotationTutorial = () => {
  const [result, setResult] = useState(null);
  const [text, setText] = useState("ফলাফল এখানে দেখাবে");

  const bracketNotationExamples = [
    ...
    { method: "obj[varWithPropertyName]", description: "ভেরিয়েবলের মান ব্যবহার করে প্রপার্টি অ্যাক্সেস করতে" },
  ];

  const executeBracketNotation = (index) => {
    let res;
    ...
    switch (index) {
      ...
      case 2:
        {
          const propName = "job";
          const job = obj[propName];
          res = `obj['${propName}'] = ${job}`;
        }
        setResult(res);
        setText(`ভেরিয়েবলের মান ব্যবহার করে প্রপার্টি অ্যাক্সেস করা হয়েছে: ${res}`);
        break;
      default:
  ...
  return (
    <div className="flex flex-col h-auto p-4">
      <div className="flex flex-col md:flex-row md:items-center">
        <div className="text-cyan-600 text-lg md:text-2xl mb-2 md:mb-0">Bracket Notation:</div>
      </div>
      ...
      <div className="mt-4 md:mt-2">
        <h3 className="text-teal-500 text-lg md:text-xl text-center">Execute Bracket Notation Operation</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
          {bracketNotationExamples.map((curMethod, index) => (
            <div
              className="bg-slate-500 text-white rounded-md flex items-center justify-center h-10 text-xs md:text-sm cursor-pointer"
              key={index}
              onClick={() => executeBracketNotation(index)}
            >
              {curMethod.method}
            </div>
          ))}
      ...
      <div className="mt-4">
        <h3 className="text-teal-500 text-lg md:text-xl">Bracket Notation Method Descriptions:</h3>
        <table className="min-w-full bg-slate-950 border border-gray-200">
          ...
          <tbody>
            {bracketNotationExamples.map((methodObj, index) => (
...
export default BracketNotationTutorial;
