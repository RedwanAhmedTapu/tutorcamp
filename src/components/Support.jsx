import React from "react";
import SupportImage from "../assets/support.png"

const Support = () => {
  return (
    <section className="w-full mt-8 md:mt-16 bg-[#F1F7FF] dark:bg-[#130e2e] overflow-hidden">
      <div className="w-full max-w-[1600px] mx-auto px-6 py-24 flex items-center justify-between gap-10 flex-col md:flex-row">
        <img
          src={SupportImage}
          alt="image"
          className="w-full md:w-[550px] bg-cover"
        />

        <div className="w-full md:w-[45%]">
          <h4 className="text-[16px] md:text-[20px] text-textColor font-[500] dark:text-yellow-50">
            Looking for support?
          </h4>
          <h2 className="text-[20px] md:text-[30px] text-textColor font-[700] mt-5 dark:text-teal-200">
            Lifetime Free Support
          </h2>
          <p className="text-[14px] md:text-[18px] font-[500] text-textColor mt-3 mb-8 dark:text-violet-50 ">
            Our main aim is to provide you the best quality education with best
            instructors. If you have any quires or you need any kinds of help
            then you can get the proper support from our very enegrgytic help &
            support team.
          </p>

          <button className="py-3 px-6 bg-violet-700 rounded-md text-[#fff]">
            Get Support
          </button>
        </div>
      </div>
    </section>
  );
};

export default Support;